...
const IconText: React.FC<{ icon: string; text: string }> = ({ icon, text }) => (
  <li className="flex gap-2">
    <Image src={icon} alt={text} className="size-4.5 md:size-5" />
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        <figure className="flex justify-end overflow-hidden">
          <Image
            ...
            height={80}
            alt="Project Cover"
            className="h-20 w-37.5 rounded-md object-cover shadow-[0px_1.66px_3.74px_-1.25px_#18274B1F]"
          />
        </figure>
      </div>

      <div>
        <div className="bg-primary text-primary-content my-4 h-25 overflow-scroll rounded-2xl px-4 py-2">
        ...
        <div className="flex gap-5">
          {livePreview && (
            <a
              ...
              target="_blank">
              <PreviewIcon className="h-auto w-4.5 md:w-5" />
          ...
          {githubLink && (
            <a
              ...
              className="text-accent flex gap-2 text-sm underline underline-offset-[3px] transition-all duration-75 ease-linear hover:scale-105 md:text-base"
              target="_blank">
              <GithubIcon className="w-4.5 md:w-5" />
